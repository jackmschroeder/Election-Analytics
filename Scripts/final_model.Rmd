...
results <- states %>% 
  mutate(R_pv2p = rep/(rep+dem),
         ...
         Guide = ifelse(R_pv2p > D_pv2p, ifelse(abs(R_pv2p - D_pv2p) > .0604, "Favor Trump", "Lean Trump"), ifelse(abs(D_pv2p - R_pv2p) < .0604, "Lean Biden", "Favor Biden")))
...
pv2p_results <- results %>% 
  left_join(., elec, by = "state") %>% 
  select(state, R_pv2p, D_pv2p, `2020`) %>% 
  mutate(D_pv2p = round(D_pv2p * 100, 1),
         R_pv2p = round(R_pv2p * 100, 1))

pv2p_results %>% 
  filter(abs(R_pv2p - D_pv2p) < 6.04) %>% 
  arrange(desc(D_pv2p))#%>% 
  #tally(`2020`)
...
shy_results %>% 
  ...
  mutate(margin = abs(R_pv2p - D_pv2p)) %>% 
  filter(margin < 0.0604) %>% 
  arrange(desc(R_pv2p))
```
...
super_shy_results %>% 
  filter(abs(R_pv2p - D_pv2p) < 0.0604) %>% 
  arrange(desc(R_pv2p - D_pv2p))
```
...
```{r reflection, echo=FALSE, message=FALSE}

# States wrong
# How off in EC votes

# Yanxi polling/econ data being off

# Method, RMSE, NY

results_20 <- read_csv("../Data/StateResults2020.csv", col_types = cols(`Kanye West` = col_double(), `Joseph R. Biden Jr.` = col_double(), `Donald J. Trump` = col_double())) %>% 
  filter(!is.na(`Donald J. Trump`)) %>% 
  rename(R_votes = `Donald J. Trump`,
         D_votes = `Joseph R. Biden Jr.`,
         state = `Geographic Name`) %>% 
  mutate(R_pv2p_20 = round(100 * R_votes/(R_votes + D_votes), 1),
         D_pv2p_20 = round(100 * D_votes/(R_votes + D_votes), 1),
         winner_actual = case_when(R_pv2p_20 > 50 ~ "Trump",
                            TRUE ~ "Biden")) %>% 
  select(state, R_pv2p_20, D_pv2p_20, R_votes, D_votes, winner_actual) 

results_20_f <- results_20 %>% 
  left_join(., pv2p_results, by = "state") %>% 
  mutate(input = (R_pv2p_20 - R_pv2p)^2,
         simple = R_pv2p_20 - R_pv2p,
         sqrt = sqrt(input),
         winner_predicted = case_when(R_pv2p > 50 ~ "Trump",
                            TRUE ~ "Biden"),
         wrong = case_when(winner_actual == winner_predicted ~ "Correct",
                           TRUE ~ "Wrong"))

results_20_k <- read_csv("../Data/StateResults2020.csv", col_types = cols(`Kanye West` = col_double(), `Joseph R. Biden Jr.` = col_double(), `Donald J. Trump` = col_double())) %>% 
  filter(!is.na(`Donald J. Trump`)) %>% 
  rename(R_votes = `Donald J. Trump`,
         D_votes = `Joseph R. Biden Jr.`,
         kanye = `Kanye West`,
  ...
  mutate(R_pv2p_20 = round(100 * R_votes/(R_votes + D_votes), 1),
         D_pv2p_20 = round(100 * D_votes/(R_votes + D_votes), 1),
         K_pv3p_20 = round(100 * kanye/(kanye + R_votes + D_votes), 1),
         winner_actual = case_when(R_pv2p_20 > 50 ~ "Trump",
                            TRUE ~ "Biden")) %>% 
  select(state, R_pv2p_20, D_pv2p_20, K_pv3p_20, kanye, R_votes, D_votes, winner_actual) %>% 
  left_join(., pv2p_results, by = "state") %>% 
  mutate(input = (R_pv2p_20 - R_pv2p)^2,
         simple = R_pv2p_20 - R_pv2p,
         sqrt = sqrt(input),
...
rmse_20 <- results_20_f %>%
  summarize(rmse = sqrt(mean(input))) %>% 
  pull()

rmse_20_ny <- results_20_f %>% 
  filter(state != "New York") %>% 
  summarize(rmse = sqrt(mean(input))) %>% 
  pull()

results_20_e <- error_results %>% 
  mutate(R_pv2p = round(R_pv2p * 100, 1),
         D_pv2p = round(D_pv2p * 100, 1)) %>% 
  select(state, R_pv2p, D_pv2p, winner) %>% 
  rename(winner_predicted = winner) %>% 
  left_join(., results_20, by = "state") %>% 
  mutate(input = (R_pv2p_20 - R_pv2p)^2,
         simple = R_pv2p_20 - R_pv2p,
         sqrt = sqrt(input),
         winner_predicted = case_when(R_pv2p > 50 ~ "Trump",
...
rmse_20_e <- results_20_e %>%
  summarize(rmse = sqrt(mean(input))) %>% 
  pull()

rmse_20_e_ny <- results_20_e %>%
  filter(state != "New York") %>% 
  summarize(rmse = sqrt(mean(input))) %>% 
  pull()

ggplot(results_20_f, aes(fill = sqrt, state = state)) +
  geom_statebins() +
  theme_statebins() +
  labs(title = "Random Forest Model Error by State",
       subtitle = "Error not geographically consolidated.",
       caption = "New York has yet to report over 90% of ballots.\nModel and graphic created by Jack Schroeder.",
       fill = "Error") +
  ggsave("../Plots/errormap20.png")

results_20_k %>% 
  filter(kanye>0) %>% 
  rename(Trump = R_votes,
         Biden = D_votes) %>% 
  pivot_longer(Trump:Biden, names_to = "party", values_to = "votes") %>% 
  ggplot(aes(x=votes, y=kanye)) +
  geom_point(color="#c8793c") +
  geom_smooth(method="lm", se=FALSE, color="#1504f8") +
  facet_wrap(~party) +
  labs(x = "Main Candidate Votes",
       y = "Kanye West Votes",
       title = "Kanye West Votes Significantly Correlated with Trump Votes",
       caption = "Trump regression: P-value 0.0007, R-squared 0.60.\nBiden regression: P-value 0.42, R-squared 0.05.") +
  theme_bw() +
  ggsave("../Plots/kanye.png",
         width = 7,
         height = 5)

# summary(lm(kanye ~ D_votes, results_20_k, subset = kanye>0))
# summary(lm(simple ~ D_votes, results_20_k))

results_20_f %>% 
  filter(state != "District of Columbia") %>% 
  ggplot(aes(x=R_pv2p, y=R_pv2p_20, size=`2020`)) +
  geom_point() +
  geom_abline(slope = 1, intercept = 0, color="salmon") +
  labs(x = "Predicted Trump Two-Party Voteshare",
       y = "Actual Trump Two-Party Voteshare",
       title = "Random Forest Model Error",
       subtitle = "Error increases with predicted Trump support",
       caption = "DC omitted from graphic.\nModel and graphic created by Jack Schroeder.",
       size = "EC Votes") +
  geom_smooth(se = FALSE) +
  theme_bw() +
  ggsave("../Plots/simpleerror.png",
         height=5,
         width=7)

results_20_f %>% 
  #filter(state != "New York") %>% 
  ggplot(aes(x=winner_predicted, y=simple)) +
  geom_boxplot(outlier.shape = NA) +
# Size should equal confidence
  geom_abline(slope = 0, intercept = 0, color = "black", alpha = 0.5) +
  geom_jitter(width=0.15, aes(color = winner_actual, size=`2020`)) +
  labs(x = "Predicted Winner",
       y = "Prediction Error",
       color = "Actual Winner",
       size = "Electoral Votes",
       caption = "Positive error means model underestimated Trump.\nColor denotes acutal winner.\nModel and graphic created by Jack Schroeder.",
       title = "Model Missed Trump Support More in Predicted Red States",
       subtitle = "RF model only overestimated Trump support in 7 states + DC") +
...
polls_error <- data_20 %>% 
  ...
  left_join(., results_20) %>% 
  select(state, avg_support_adjusted_2, avg_support_adjusted_10, R_pv2p_20, winner_actual) %>% 
  rename(`Poll Avg.\n(>2 Weeks)` = avg_support_adjusted_10,
         `Poll Avg.\n(<2 Weeks)` = avg_support_adjusted_2,
         `Actual\nResults` = R_pv2p_20) %>% 
  mutate(error = `Actual\nResults` - `Poll Avg.\n(<2 Weeks)`,
         errorfive = ifelse(error >= 5, 1, 0)) %>% 
  pivot_longer(`Poll Avg.\n(<2 Weeks)`:`Actual\nResults`, names_to = "indicator", values_to = "pestimate") %>% 
  mutate(indicator = fct_relevel(indicator, "Poll Avg.\n(>2 Weeks)", "Poll Avg.\n(<2 Weeks)", "Actual\nResults"),
         pestimate_50 = ifelse(pestimate > 50, "Trump", "Biden"))
...
polls_error %>% 
  ...
  facet_wrap(~winner_actual) +
  labs(x = "",
       y = "Trump Voteshare",
       title = "Polling Averages Underestimated Trump",
       subtitle = "Averages were fairly steady pre-election, but actual results shifted red",
       color = "Leader",
       caption = "Polling averages from FiveThirtyEight.\n>2 week averages extend up to 10 weeks pre-election.\nModel and graphic created by Jack Schroeder.") +
  theme_bw() +
  ggsave("../Plots/pollavg.png")

correlates <- data_20 %>% 
  filter(party == "republican") %>% 
  left_join(., results_20, by = "state") %>% 
  #mutate(my_pv2p_20 = ifelse(party == "republican", R_pv2p_20, D_pv2p_20)) %>% 
  select(state, party, R_pv2p_20, my_pv2p_lag, avg_support_adjusted_10, avg_support_adjusted_2, unemployment_loc_a) %>%
  rename(`Poll Avg.\n(>2 Weeks)` = avg_support_adjusted_10,
         `Poll Avg.\n(<2 Weeks)` = avg_support_adjusted_2,
         `2016 Results` = my_pv2p_lag) %>% 
  pivot_longer(`2016 Results`:`Poll Avg.\n(<2 Weeks)`, names_to = "indicator", values_to = "pestimate")

correlates %>% 
  filter(state != "District of Columbia",
         indicator != "unemployment_loc_a") %>% 
  ggplot(aes(x=pestimate, y=R_pv2p_20)) +
  geom_point() +
  geom_smooth(se = FALSE, alpha=0.5) +
  geom_abline(slope=1, intercept=0, color="salmon") +
  facet_wrap(~indicator) +
  labs(x = "Predicted Trump Voteshare",
       y = "Actual Trump Voteshare",
       title = "Lagged Voteshare Overestimated Trump Support",
       subtitle = "But polling underestimated Trump support by wider margins",
       caption = "Polling averages from FiveThirtyEight.\n>2 week averages extend up to 10 weeks pre-election.\nModel and graphic created by Jack Schroeder.") +
  theme_bw() +
  ggsave("../Plots/correlates.png",
         height = 5,
         width = 7)
