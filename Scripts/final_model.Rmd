...
results_20 <- read_csv("../Data/popvote_bystate_1948-2020.csv") %>% 
  filter(year == 2020) %>% 
  mutate(R_pv2p = round(R_pv2p * 100, 1),
         D_pv2p = round(D_pv2p * 100, 1),
         winner_actual = case_when(R_pv2p > 50 ~ "Trump",
                                   TRUE ~ "Biden")) %>% 
  left_join(., pv2p_results, by = "state", suffix = c("_actual", "_predicted")) %>% 
  mutate(winner_predicted = case_when(R_pv2p_predicted > 50 ~ "Trump",
                            TRUE ~ "Biden"),
         input = (R_pv2p_actual - R_pv2p_predicted)^2,
         simple = R_pv2p_actual - R_pv2p_predicted,
         sqrt = sqrt(input),
...
rmse_20_ny <- results_20 %>% 
  filter(state != "New York") %>% 
  summarize(rmse = sqrt(mean(input))) %>% 
  pull()

ggplot(results_20, aes(fill = sqrt, state = state)) +
  geom_statebins() +
  theme_statebins() +
...
# summary(lm(kanye ~ D_votes, results_20, subset = kanye>0))
