...
results %>% 
  ...
  geom_statebins() +
  theme_statebins() +
  scale_fill_manual(values = c("#0000ff", "#ff0000", "#add8e6", "#ffcccb"),
...
# Create other comparison models - can include the REALLY bad state models from earlier - justify move to RF because of obviously incorrect predictions in draws/victory margin

# Within RF - compare to just average support (unadjusted) - probably no difference

# Need to quantify uncertainty - probably around 3 points is my guess

# PV vs PV2P - fix this first, rationale is that pv2p don't add up to 100

# Show RF with predicting R and D - really bad r-squared

# Explain model validation

# Explain model parameters - what is the final model?

# Interpretation of model - downside is that it is sort of a black box here

```

```{r undecided, echo=FALSE, message=FALSE}
