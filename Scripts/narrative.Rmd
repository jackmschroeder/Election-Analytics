...
map1 <- left_join(cor16, cor20, by = c("state", "party"), suffix = c(".16", ".20")) %>% 
  mutate(delta = real_error.20 - real_error.16,
         `Error Consistency` = case_when(abs(real_error.20) > 7 & abs(real_error.16) > 7 ~ "Alarming (>7)",
                            abs(real_error.20) > 4 & abs(real_error.16) > 4 ~ "Worrying (>4)",
                            TRUE ~ "Tolerable (<4 Points)"))

ggplot(map1, aes(fill = `Error Consistency`, state = state)) +
  geom_statebins() +
  theme_statebins() +
  scale_fill_manual(values = c("#8ace7e", "#ffda66", "#ff684c"),
                    breaks = c("Tolerable (<4 Points)", "Worrying (>4)", "Alarming (>7)")) +
  labs(title = "Error Correlation Map",
       subtitle = "Most States Off by >4 Points in Both Elections.",
       caption = "Polling averages missed red and yellow states by >7 and >4 points, respectively, in both elections.\nGreen states may still have seen polling misses, but not consistently >4 points.\nModel and graphic created by Jack Schroeder.") +
