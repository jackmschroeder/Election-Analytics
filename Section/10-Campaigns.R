...
speech_df <- read_csv("R/Election-Analytics/Data/campaignspeech_2019-2020.csv")
...
tweet_df <- read_csv("Data/trumptweets_2016-2020.csv")

## pre-process: make a `quanteda` corpus from dataframe
tweet_corpus <- corpus(tweet_df, text_field = "text", docid_field = "id")
...
## pre-process: make doc-freq matrix
tweet_dfm <- dfm(tweet_toks, groups = "isRetweet")

## summarise and visualise
tweet_tstat_freq <- textstat_frequency(tweet_dfm)
head(tweet_tstat_freq, 100)

textplot_wordcloud(tweet_dfm, color = c("red", "blue"), comparison = T)

trump_keyness <- textstat_keyness(tweet_dfm, target = "TRUE")
textplot_keyness(trump_keyness)
